...
[fit_params, ~] = fit_resonance_circle(freq, S21, 'show_plot', 1, 'robust_phase', true);

[tau, alpha, A, ~] = calibrate_resonance(freq, S21, 0, []);
S21_calibrated = apply_calibration(tau, alpha, A, freq, S21);
S21_cal_dB = 20*log10(abs(S21_calibrated));

figure('Name','Calibrated S21 (Baseline 0 dB)');
plot(freq/1e9, S21_cal_dB,'LineWidth',1.5);
...
plot(real(S21_calibrated), imag(S21_calibrated), '.', 'MarkerSize', 15);
